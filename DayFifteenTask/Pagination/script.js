...
div.setAttribute("id", "div-table");
div.classList.add("table-responsive"); //ClassList (div new class)
body.append(div);
...
div2.setAttribute("class", "outer-shadow hover-in-shadow");
div1.append(div2);
body.append(div1);
...
function pagination(querySet, pages, rows) {
...
function pageButtons(pages) {
  ...
  console.log("maxRigth", maxRight);

  for (var page = maxLeft; page <= maxRight; page++) {
    wrapper.innerHTML =
      wrapper.innerHTML +
      `<button id=${page} onclick="btn(this)" class="page btn btn-sm btn-info">${page}</button>`;
  }

  if (state.page != 1) {
    wrapper.innerHTML =
      `<button id=${1} onclick="btn(this)" class="page btn btn-sm btn-info"> << </button>` +
      `<button id=${
        state.page - 1
      } onclick="btn(this)" class="page btn btn-sm btn-info"><</button>` +
      wrapper.innerHTML;
  }
  ...
  if (state.page != pages) {
    wrapper.innerHTML =
      ...
      `<button id=${pages} onclick="btn(this)" class="page btn btn-sm btn-info"> >></button>`;
  }
}

function btn(e) {
  let k = document.getElementById("table-body");
  let parent = k.parentNode;
  parent.removeChild(k);
  console.log("e", e.id);
  state.page = e.id;
  console.log("stage page", state.page);
  buildTable();
}
...
function buildTable() {
  ...
  for (let i = 0; i < myList.length; i++) {
    ...
    for (let j of Object.keys(myList[i])) {
      let td = document.createElement("td");
      td.innerText = myList[i][j];
      tr.append(td);
    }
    tbody.append(tr);
  }
